<div class="form-group">
    <x-form.input label="Product Name" class="form-control-lg" role="input" name="name" :value="$category->name" />
</div>
<div class="form-group">
    ...
    <select name="category_id" class="form-control form-select">
        ...
        @foreach (App\Models\Category::all() as $categoryItem)
            <option value="{{ $categoryItem->id }}" @selected(old('category_id', $category->parent_id) == $categoryItem->id)>{{ $categoryItem->name }}</option>
        @endforeach
    </select>
</div>
